...
⚠ **enum 컬럼의 CHECK 제약 함정** — `@Enumerated(EnumType.STRING)`을 쓰면 Hibernate 6이
CHECK 제약을 자동 생성한다(`VerifyDbRole.java`가 `status='CHECK'`로 이미 한 번 밟았다).
`ddl-auto: update`는 최초 8개 값으로 제약을 만들고 **enum이 늘어나도 고쳐주지 않는다.**
나중에 `NATURE`·`TEMPLE`을 추가하면 적재 도중 제약 위반으로 죽는데, 그게 권한 오류나
매핑 오류처럼 보인다. 값을 추가할 때는 제약을 직접 `alter`해야 한다.
...
- `overview`에서 뽑은 사실은 `evidence.url`에 **`detailCommon2` 호출 URL을 그대로** 싣는다.
  §9의 완료조건이 이 문자열로 질의하므로, 계약이 이걸 강제하지 않으면 검증 자체가 불가능하다
...
### 6.3 ★ 의심 구간 — 역방향 시딩이 중복 노드를 만드는 경로를 먼저 막는다

**이걸 안 고치면 역방향 시딩이 레지스트리를 다시 오염시킨다.** 2026-08-05에 별칭 오염으로
레지스트리를 통째로 지운 것과 같은 부류의 실패다.

`PlaceResolver.resolve()`를 역방향 시딩이 만들어내는 바로 그 입력으로 따라가 보면:

```
이름 정확 일치 · 좌표 있음 · 그런데 두 소스 좌표가 200m 넘게 벌어짐
  → within 이 빈다
  → maybeCreate("name matched but nearest candidate is beyond radius")
  → tour_api 단서는 외부 ID가 있다 → 새 노드 생성
  → 경고 없음 · rejects 없음
```

`name_normalized`가 같은 행이 둘이 된다. 그 다음부터 이 장소에 오는 모든 단서는
`candidatesByName`에서 후보 2건을 물어와 **영영 ambiguous 거절**이거나, 200m 안에
하나만 걸리면 **그쪽으로 오병합**된다. 경복궁은 한 장소가 400m 넘게 퍼져 있고,
역방향 시딩이 닿으려는 대상이 바로 경복궁 같은 곳이다.

역방향 시딩은 정확 이름 일치를 **최대화**하는 기법이라 이 경로가 드물게가 아니라 상시로 밟힌다.

**대책 — 사다리에 의심 구간을 넣는다:**

| 이름 일치 + 거리 | 판정 |
|---|---|
| ≤ `radius-meters` (200m) | 병합 확정 (현행) |
| 200m ~ `suspect-radius-meters` (2km), 같은 city·district | **미해결 보관함** — 추측하지 않는다 |
| > 2km | 새 노드 (동명 체인·타지역 동명 장소) |

의심 구간을 새 노드로 만들지도, 병합하지도 않는 이유는 둘 다 틀릴 수 있고
**틀린 쪽의 비용이 비대칭**이기 때문이다. 미해결은 원본을 그대로 보관하므로 언제든 되살릴 수 있다.

- 새 설정 `ingest.resolver.suspect-radius-meters` (기본 2000)
- 2km 초과를 여전히 새 노드로 두는 이유: 서로 다른 도시의 동명 장소·체인점은 진짜로 별개다.
  이걸 미해결로 보내면 정상 시딩이 전부 보관함으로 새 나간다
- **역방향 시딩으로 얻는 병합이 줄어드는 것은 의도된 대가다.** 경복궁급 대형 부지는
  자동 병합 대신 보관함에 눈에 보이게 남는다. 조용한 중복 노드보다 낫다.
  범위별 `radius-meters` 조정이 이걸 푸는 레버다
- 필수 테스트: **이름 정확 일치 + 500m → 두 번째 노드가 생기지 않는다**
...
- Kakao 키가 없으면 `kakaoEnabled=false` — 단, 레지스트리는 여전히 동작해야 한다
  ...
  ⚠ **구 중심 좌표가 Kakao에 물려 있다.** `CourseRecommendController.java:101`이
  `kakaoClient.geocodeRegion()`으로 구 앵커를 구하므로, 키가 없으면 최근접 이웃의
  출발점이 없다. 도시 단위는 `KoreanCity`가 좌표를 들고 있어 괜찮지만 구 단위는 안 된다.
  → **구 앵커는 해당 구 레지스트리 장소들의 무게중심으로 계산한다.** 이미 가진 데이터로
  풀리고, Kakao 없이도 구 단위가 동작하며, 실제 수집된 장소 분포를 반영해 도리어 정확하다.
  Kakao가 있으면 기존 지오코딩을 쓴다(레지스트리가 빈 구에서도 동작해야 하므로)
...
| **의심 구간** (6.3) | 이름 정확 일치 + 500m → **두 번째 노드가 생기지 않고 미해결로 간다**. 200m 이내는 병합, 3km는 새 노드 |
| 구 앵커 | Kakao 없을 때 해당 구 레지스트리 장소들의 무게중심. Kakao 있으면 지오코딩 사용 |
