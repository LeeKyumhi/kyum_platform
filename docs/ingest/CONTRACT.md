...
`tour_api`는 역방향 시딩으로만 수집하므로, `state/registry-places.jsonl`에 그 범위 줄이
하나도 없으면 **구조적으로 0건**이다. 그런 쌍은 고르지 않고 다음 후보로 넘어간다
(대개 같은 범위의 `kakao_local` — 그게 씨앗을 심는 순서다). 건너뛴 쌍은 완료로 기록하지 않는다.

> 실측: v5 첫 실행이 `Seoul/종로구 × tour_api`를 골라 27초 만에 0건으로 끝났다.
> 규칙을 정확히 지킨 결과였고, 고치지 않으면 매 실행이 같은 자리에서 반복된다.

### §16-1. 사진 없는 기존 장소 갱신 (v6)

`registry-places.jsonl`의 `has_tour_api_id: true` + `has_image: false` 장소는
그 줄의 `tour_api_content_id`로 **`detailCommon2`를 직접 호출**해 사진만 채운다.
검색 단계가 없어 한 곳당 호출 1회다. 외부 ID로 병합되므로 새 노드가 생기지 않는다.

> 실측: 중구 53곳 중 21곳이 tour_api id를 이미 갖고 있었고, 역방향 시딩이 그 21곳을
> 건너뛰도록 되어 있어 v5 실행 뒤에도 사진은 1건뿐이었다.
