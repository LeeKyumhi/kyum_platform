...
## 0. 재개 지점 (2026-08-15) — 사진이 도달하지 못하던 화면 3곳, 브랜치 `feat/place-media-ui-followups`
...
| 곳 | 있었던 일 | 고친 방법 |
|---|---|---|
| `/spots/[slug]` 주변 장소 | 늘 이모지 타일 | `PlaceController.nearby()`가 `attachMedia` 호출. 카드 썸네일·개수 배지(`/explore` 마크업 재사용), 카드 클릭 → `PlaceDetailModal`(외부 카카오맵 링크는 모달 보조 버튼으로 흡수). 칩에 관광명소·문화시설 추가(우리 사진이 있는 종류가 그쪽이다) → 제목을 `주변 가볼 곳`으로 ko/en/zh 수정 |
| `/saved` 찜한 장소 | 늘 이모지 타일 | `/explore`가 `image: null`로 저장하고 있었다 → `coverPhotoUrl ?? officialPhotoUrl`. `SavedGrid`는 원래부터 렌더할 준비가 돼 있었다 |
| 일정 블록 → 상세 모달 | 🏛 공식 사진이 사라짐 | `GET /api/places/media` 신설(공개, `PlaceMediaLookup` 재사용). 모달은 **공식 사진을 props로 못 받았을 때만** 이 경로로 되묻는다 |

곁가지로 **같은 결함 두 곳**을 막았다 — `ModalPlace.id`는 카카오 장소 id인데,
① `openDetail`이 아이템 키(`new-3`)를, ② `ChatRoom.openPlaceDetail`이 **메시지 id**를 거기 넣고 있었다.
모달은 그 값으로 노트를 조회하고 **작성할 때도 그대로 싣는다** — 어느 장소에도 안 붙는 행이 생기고,
②는 숫자가 겹치면 엉뚱한 장소에 붙을 수도 있다. 이제 ①은 `it.placeId ?? ""`, ②는 카카오맵 URL에서
id를 되찾고 없으면 빈 값이다. 식별자가 없으면 모달이 노트 작성 버튼 자체를 숨긴다.

**폴백 조회는 `undefined`일 때만 돈다** — 목록에서 온 장소는 사진이 없어도 키가 `null`로 실려 온다
(서버가 "없다"고 답한 것). `!officialPhotoUrl`로 재면 사진 없는 장소마다 왕복이 하나씩 붙는다
(첫 구현이 그랬고 Playwright 로그에서 잡았다). 키가 아예 없는 경로(일정 블록·채팅 장소카드)만 되묻는다.

**검증** — `tsc --noEmit` 0 · **346 tests / 0 failures** · 백엔드+프론트 실기동 + Playwright:
`/spots/gyeongbokgung` 칩 4개·카드 15개·클릭 시 모달, `/explore` 서울에서 하늘전망대 썸네일 렌더,
**목록 경로에서는 `media`를 안 부르고 `notes`만 부르는 것**까지 확인. `curl`로 `nearby`가 실제
`coverPhotoUrl`을 싣는 것 확인(`category=하늘전망대`로 조회 시 사진 1건, `photoCount=2`),
`/api/places/media`는 **비로그인 200**.

**로그인이 필요했던 2가지도 사용자 계정으로 확인 완료(2026-08-15).**
① 찜 → `/saved`에 사진(에이전트는 요청 본문에 `"image":"…_thumb.jpg"`가 실리는 데까지만 증명했었다).
**⚠ 스냅샷이라 이미 찜해둔 장소는 해제 후 재찜해야 바뀐다.**
② 일정에 담고 새로고침 후 블록 탭 → 🏛 공식 사진.
②가 보였다는 것은 **`kakao_place_id`와 `image_url`을 둘 다 가진 장소가 실제로 있다**는 뜻이다
(에이전트는 DB를 못 봐서 이 사실을 확정하지 못했었다 — 다음 세션은 이걸 전제해도 된다).

> **⚠ 새로 안 사실: 에이전트가 만든 계정으로는 로그인이 불가능하다.**
> `AuthService:177`의 이메일 인증 게이트가 LOCAL 계정을 막고(403 `EMAIL_NOT_VERIFIED`),
> 인증 토큰은 Resend 메일로만 나가며 플래그로 끌 수 없다. 인증 메일은 Resend 계정 소유자
> 주소로만 발송된다. **로그인이 필요한 검증은 사용자의 인증된 계정으로만 가능하다**
> (스모크 스크립트들이 `<이메일인증계정> <비번>`을 인자로 받는 이유가 이것이다).
> 이번에 시도하다 쓸모없는 행 1개가 생겼다 — `media_smoke_1786791810@test.com`
> (`%_smoke_%` 정리 쿼리에 잡히도록 이름 지음). 아래 정리 목록에 추가.

**⚠ 알려진 한계(의도)** — 레지스트리 전용 장소(kakao id 없음)를 일정에 담으면
`placeModuleAt`이 식별자를 저장하지 않아 **새로고침 뒤에는 되물을 키가 없다.**
고치려면 `ItineraryItem`에 컬럼이 필요하다(스키마 변경이라 이번 범위 밖).

### 같은 브랜치에서 고친 버그: 여행 일정 페이지 스크롤 먹통

사용자 신고 — "내 여행계획 짜기 스크롤이 먹통". 원인은 이번 작업과 무관한 기존 문제였다.
`TripMap`이 카카오 지도를 `scrollwheel` 옵션 없이 만드는데, **카카오 기본값이 휠=확대/축소**다.
지도가 휠을 가로채고 preventDefault까지 하므로 **커서가 지도 위에 있으면 페이지가 안 움직인다.**
`/trips/[id]`는 시간표 바로 아래에 이 256px 지도가 있어 내려가다 반드시 걸린다.

측정(공개 경로 `/explore` 상세 모달의 같은 컴포넌트, 로그인 없이 재현):

| | 고치기 전 | 고친 뒤 |
|---|---|---|
| 지도 **위**에서 휠 | scrollTop 0 → 0 (먹통) | 0 → 24 (정상) |
| 지도 **밖**에서 휠 | 0 → 24 (정상) | 0 → 24 (정상) |

`scrollwheel: false` + 조작 수단으로 `ZoomControl` 추가(스크린샷으로 +/− 렌더 확인, 콘솔 에러 0).
**지도를 쓰는 모든 화면에 함께 적용된다** — `/trips/[id]`·상세 모달·가이드 코스 동선.
먼저 의심했던 "모달이 body 스크롤 잠금을 안 푼다"는 Esc·백드롭·✕ 세 경로 전부 원복되는 것을
확인해 배제했다. ⚠ **모바일 터치는 별개다** — 한 손가락 드래그는 여전히 지도 패닝이 먹는다(미조치).

**`/api/places/search`는 일부러 안 붙였다** — 채팅 만남장소 피커라 결과물(장소 카드)에
사진이 실리지 않고, 검색마다 쿼리 2~3회를 얹을 값이 없다. 이월 항목의 "확인 필요"는 이 판단으로 닫는다.
...
- ~~`/api/places/nearby`·`/api/places/search`는 대표 사진을 안 붙인다~~ → **2026-08-15 확인·처리 완료.**
  Task 11이 손댄 경로(`/explore`·팔레트)는 둘 다 `/api/places`라 정상이었다. 실제 구멍은
  `nearby`(=`/spots/[slug]`) 하나였고 이번에 배선했다. `search`는 의도적으로 안 붙인다(위 최상단 절 참고).
