...
1. **`/spots/[slug]` 주변 장소** — `PlaceController.nearby()`에 `attachMedia` 1줄.
   `recommendFirst`는 일부러 안 부른다(이 목록에서 순서가 뜻하는 건 "명소에서 가까운 순"이고,
   그 축을 추천 신호로 갈아끼우면 섹션이 답하던 질문이 사라진다).
   프론트는 `/explore` 카드 마크업 재사용(썸네일+개수 배지) + 카드 클릭 → `PlaceDetailModal`.
   외부 카카오맵 버튼은 모달 보조 버튼으로 흡수(Wave 4가 `/explore`에서 내린 결정).
   칩에 관광명소·문화시설 추가 — 우리가 사진·지식을 가진 종류가 그쪽이라 식당·카페만 두면
   **이 배선이 화면에서 증명되지 않는다.** i18n 키는 기존 `explore.cat*` 재사용, 제목만
   `주변 맛집·카페` → `주변 가볼 곳`(ko/en/zh)으로 수정.
2. **`/saved` 찜 그리드** — `/explore`가 `image: null`을 저장하고 있었다.
   `coverPhotoUrl ?? officialPhotoUrl ?? null`. ⚠ **저장 시점 스냅샷**이라(`name`·`address`와 같은 성질)
   이미 찜해둔 행은 안 바뀐다 — 재찜해야 한다. 재조회 로직은 일부러 안 넣었다.
3. **일정 블록 → 상세 모달의 공식 사진** — 시간표 아이템에는 사진 필드가 없어 담는 순간 사라졌다.
   `GET /api/places/media`(공개, `PlaceMediaLookup` 재사용, 규칙 복제 없음) 신설 +
   모달은 **props로 못 받았을 때만** 되묻는다(기존 경로는 왕복 증가 0).
   ⚠ `SecurityConfig`의 permitAll은 **경로 정확 일치 목록**이라 새 GET은 반드시 거기 등록해야 한다.

**곁가지 결함 두 곳(같은 종류)** — `ModalPlace.id`는 카카오 장소 id인데 ① `openDetail`이 아이템 키
(`new-3`)를, ② `ChatRoom.openPlaceDetail`이 **메시지 id**를 넣고 있었다. 모달은 조회만이 아니라
**작성할 때도** 그 값을 싣는다 → 어느 장소에도 안 붙는 행이 생기고, ②는 숫자가 겹치면 엉뚱한 장소에
붙는다. ①은 `it.placeId ?? ""`, ②는 카카오맵 URL(`place.map.kakao.com/{id}`)에서 되찾고 없으면 빈 값.
식별자가 없으면 노트 작성 버튼을 숨긴다.

**폴백 조회 조건은 `undefined`다(`!url`이 아니라)** — 목록에서 온 장소는 사진이 없어도 키가 `null`로
실려 온다(서버가 "없다"고 답한 것). 처음엔 `if (place.officialPhotoUrl) return`으로 짰는데,
사진 없는 장소가 대부분이라 **거의 모든 모달이 쓸데없는 왕복을 하나씩** 더 하고 있었다 —
Playwright 네트워크 로그로 잡았다. 키가 아예 없는 경로(일정 블록·채팅 장소카드)만 되묻는다.

**검증** — tsc 0 · **346 tests / 0 failures** · 실기동 + Playwright:
`/spots/gyeongbokgung` 칩 4개·카드 15개·카드 클릭 시 모달, `/explore` 하늘전망대 썸네일,
목록 경로에서 `media`를 안 부르고 `notes`만 부름, 찜 요청 본문에 `image` 실림.
curl로 `nearby`가 `coverPhotoUrl`을 싣는 것 실측(`photoCount=2`), `/api/places/media` 비로그인 200.

**⚠ 에이전트는 로그인할 수 없다** — 이메일 인증 게이트(`AuthService:177`)가 LOCAL 계정을 막고
인증 토큰은 Resend 메일로만 나간다. 플래그도 없다.
(기존 스모크 스크립트들이 `<이메일인증계정> <비번>`을 인자로 받는 이유가 이것이다.)
그래서 마지막 2건은 **사용자가 직접 확인했다(2026-08-15)** — 찜 후 `/saved` 사진 ✅,
담은 뒤 새로고침한 블록의 🏛 공식 사진 ✅. 후자가 보였다는 건 `kakao_place_id`와 `image_url`을
**둘 다 가진 장소가 실재한다**는 뜻이다(에이전트는 DB를 못 봐 확정하지 못했던 부분).

**알려진 한계(의도)** — 레지스트리 전용 장소는 담길 때 식별자가 저장되지 않아 새로고침 후 되물을
키가 없다. `ItineraryItem` 컬럼이 필요해 이번 범위 밖.

**같은 브랜치에서 고친 버그 — 여행 일정 페이지 스크롤 먹통** (사용자 신고, 기존 문제)

`TripMap`이 `new kakao.maps.Map()`을 `scrollwheel` 없이 부른다 → **카카오 기본값이 휠=확대/축소**라
지도가 휠을 가로채고 preventDefault한다. 커서가 지도 위면 페이지가 안 움직인다. `/trips/[id]`는
시간표 바로 아래에 256px 지도가 있어 스크롤 경로에 반드시 걸린다.
`scrollwheel: false` + `ZoomControl`(휠 확대를 껐으니 조작 수단은 남겨야 한다).

- 재현·검증은 **로그인 없이** 했다 — 같은 컴포넌트를 쓰는 공개 경로(`/explore` 상세 모달)에서
  지도 위/밖 휠의 `scrollTop` 변화를 비교. 전: 위 0→0 / 밖 0→24. 후: 위 0→24 / 밖 0→24.
- 오진 하나를 먼저 배제했다: "모달이 body 스크롤 잠금을 안 푼다" → Esc·백드롭·✕ 세 경로 모두
  `body.style.overflow`가 원복되는 것을 실측(`useModalDismiss`는 정상).
- ⚠ **모바일 터치는 미조치** — 한 손가락 드래그는 여전히 지도 패닝이 먹는다. 고치려면 탭-활성화나
  두 손가락 패닝 같은 별도 패턴이 필요하다(사용자 신고는 데스크탑 휠이었다).
...
- **백로그(최종 리뷰 triage)**: ~~① counts ids 무제한+인덱스 없음~~ ~~③ guides ♥뱃지 즉석 갱신 안 됨~~ ~~④ followCourse 고아 일정~~ ~~⑦ aria-label 정적~~ → **2026-07-20 `chore/wishlist-backlog`로 해소·main 머지** (ids 100 상한+`idx_saved_items_type_ref`+프론트 100청크 분할, SAVED_CHANGED_EVENT 구독 재집계, PUT 실패 시 best-effort DELETE, aria-label=t.saved.saveBtn/savedBtn). **의도적 수용(잔존)**: ② 서비스 레벨 exists→save TOCTOU(Follow 패턴 동일) ⑤ `/saved` fetch 실패=빈상태 구분 불가(following 페이지 패턴) ⑥ 해제 시 stale row 간헐(1/5, 이벤트 refetch 의존).
